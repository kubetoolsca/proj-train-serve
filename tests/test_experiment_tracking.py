"""Lightweight tests for experiment tracking artifacts.

These tests validate that run directories, configs, metrics, metadata,
checkpoint callbacks, and TensorBoard logger paths are created correctly.
No Fashion-MNIST download or actual training is required.
"""

import json
from unittest.mock import patch

from lightning.pytorch.callbacks import ModelCheckpoint
from lightning.pytorch.loggers import TensorBoardLogger

from src.image_classifier.pytorch.train import (
    _build_config,
    _collect_run_metadata,
)


# Helpers
def _make_fake_args(**overrides):
    """Return a namespace mimicking parsed CLI arguments."""

    class Args:
        data_dir = "data"
        output_dir = "outputs"
        batch_size = 64
        learning_rate = 0.001
        epochs = 5
        num_workers = 0
        seed = 42
        run_name = "fashion-mnist-lightning"
        accelerator = "cpu"
        devices = 1

    args = Args()
    for key, value in overrides.items():
        setattr(args, key, value)
    return args


# Tests
def test_run_directory_creation(tmp_path):
    """Run directory is created with the correct name format."""
    run_id = "2026-07-02-120530-fashion-mnist-lightning"
    run_dir = tmp_path / "runs" / run_id
    run_dir.mkdir(parents=True, exist_ok=True)

    assert run_dir.exists()
    assert run_dir.is_dir()
    assert run_dir.name == run_id


def test_config_json_writing(tmp_path):
    """config.json contains all 10 required keys with correct values."""
    args = _make_fake_args()
    config = _build_config(args)

    config_path = tmp_path / "config.json"
    with open(config_path, "w") as f:
        json.dump(config, f, indent=2)

    loaded = json.loads(config_path.read_text())

    expected_keys = {
        "data_dir",
        "dataset",
        "framework",
        "model",
        "batch_size",
        "learning_rate",
        "epochs",
        "num_workers",
        "seed",
        "accelerator",
        "devices",
    }
    assert set(loaded.keys()) == expected_keys
    assert loaded["dataset"] == "fashion-mnist"
    assert loaded["framework"] == "pytorch-lightning"
    assert loaded["model"] == "simple-cnn"
    assert loaded["batch_size"] == 64
    assert loaded["learning_rate"] == 0.001
    assert loaded["accelerator"] == "cpu"
    assert loaded["devices"] == 1


def test_metrics_json_writing(tmp_path):
    """metrics.json contains all 4 spec-compliant keys."""
    metrics = {
        "best_val_loss": 0.31,
        "best_val_accuracy": 0.89,
        "final_test_loss": 0.34,
        "final_test_accuracy": 0.88,
    }

    metrics_path = tmp_path / "metrics.json"
    with open(metrics_path, "w") as f:
        json.dump(metrics, f, indent=2)

    loaded = json.loads(metrics_path.read_text())

    expected_keys = {
        "best_val_loss",
        "best_val_accuracy",
        "final_test_loss",
        "final_test_accuracy",
    }
    assert set(loaded.keys()) == expected_keys

    for value in loaded.values():
        assert isinstance(value, float)


def test_run_metadata_json_writing(tmp_path):
    """run_metadata.json contains all 8 fields; git fields are nullable."""
    run_id = "2026-07-02-120530-fashion-mnist-lightning"

    with patch(
        "src.image_classifier.pytorch.train._collect_git_metadata",
        return_value={
            "git_commit": None,
            "git_branch": None,
            "git_is_dirty": None,
        },
    ):
        metadata = _collect_run_metadata(run_id)

    metadata_path = tmp_path / "run_metadata.json"
    with open(metadata_path, "w") as f:
        json.dump(metadata, f, indent=2)

    loaded = json.loads(metadata_path.read_text())

    expected_keys = {
        "run_id",
        "created_at",
        "git_commit",
        "git_branch",
        "git_is_dirty",
        "python_version",
        "torch_version",
        "lightning_version",
    }
    assert set(loaded.keys()) == expected_keys
    assert loaded["run_id"] == run_id
    assert loaded["git_commit"] is None
    assert loaded["git_branch"] is None
    assert loaded["git_is_dirty"] is None
    assert loaded["python_version"] is not None
    assert loaded["torch_version"] is not None
    assert loaded["lightning_version"] is not None


def test_checkpoint_callbacks_configuration(tmp_path):
    """Both ModelCheckpoint callbacks are configured correctly."""
    ckpt_dir = str(tmp_path / "checkpoints")

    best_cb = ModelCheckpoint(
        dirpath=ckpt_dir,
        filename="best",
        monitor="val_loss",
        mode="min",
        save_top_k=1,
    )
    last_cb = ModelCheckpoint(
        dirpath=ckpt_dir,
        filename="last",
        every_n_epochs=1,
    )

    assert best_cb.filename == "best"
    assert best_cb.monitor == "val_loss"
    assert best_cb.mode == "min"
    assert best_cb.dirpath == ckpt_dir

    assert last_cb.filename == "last"
    assert last_cb.dirpath == ckpt_dir


def test_tensorboard_logger_path(tmp_path):
    """TensorBoardLogger save_dir matches the expected run path."""
    tb_dir = str(tmp_path / "tensorboard")

    logger = TensorBoardLogger(
        save_dir=tb_dir,
        name="",
        version="",
    )

    assert logger.save_dir == tb_dir
